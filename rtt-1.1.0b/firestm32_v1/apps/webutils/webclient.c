...
static err_t hc_connected(void *arg, struct tcp_pcb *pcb, err_t err)
{
    ...
    {	
        ...
        rt_sprintf(headers,"GET /%s HTTP/1.0\r\n\r\n", state->Page);
    }
    else
    {
...
{
	rt_kprintf("num:%d errormsg:%d pagelen:%d\n page:%s \n", num,errormsg,page,len);
	if(page) rt_kprintf("%s\n\n\n", page);
	else 
	{	
		rt_kprintf("error: ");
		if(errormsg == OUT_MEM) rt_kprintf("OUT MEM");
		else if(errormsg == TIMEOUT) rt_kprintf("TIME OUT");
		else if(errormsg == NOT_FOUND) rt_kprintf("NO 200 OK");
		else if(errormsg == GEN_ERROR) rt_kprintf("GEN ERROR");
		else rt_kprintf("?? ERROR");
		rt_kprintf("\n");
	}
}
...
{
	struct parsed_url * purl = (struct parsed_url *)callback_arg;
	if(ipaddr)
	{
		rt_kprintf("IP hostname %s found ...\n", name);
	    rt_kprintf("IP: %d.%d.%d.%d \n",ip4_addr1(ipaddr),ip4_addr2(ipaddr),ip4_addr3(ipaddr),ip4_addr4(ipaddr));
		// Now we can request our page
		rt_kprintf("Get webpage ...\n");
	    hc_open(*ipaddr, purl->path, NULL, page_recv);	
	}
	else rt_kprintf("IP hostname %s NOT found ...\n", name);
	if (purl) parsed_url_free(purl);
}

void dns_found2(const char *name, struct ip_addr *ipaddr, void *callback_arg)
{
	if(ipaddr)
	{
		rt_kprintf("IP hostname %s found ...\n", name);
...
void open_url(char * url)
{
	...
	rt_kprintf("  path: %s \n",purl->path);
	rt_kprintf("  query: %s \n",purl->query);
	rt_kprintf("  fragment: %s \n",purl->fragment);
	rt_kprintf("  username: %s \n",purl->username);
	rt_kprintf("  password: %s \n",purl->password);
	
	//dns_gethostbyname(purl->host, malloc(sizeof(struct ip_addr)), dns_found, (void *)purl);
	if (dns_gethostbyname(purl->host, &ip, dns_found, (void *)purl) == ERR_OK)
	{
		rt_kprintf("IP hostname %s found ...\n", purl->host);
	    rt_kprintf("IP: %d.%d.%d.%d \n",ip4_addr1(&ip),ip4_addr2(&ip),ip4_addr3(&ip),ip4_addr4(&ip));
		// Now we can request our page
		rt_kprintf("Get webpage ...\n");
	    hc_open(ip, purl->path, NULL, page_recv);	
...
void get_hostip(char * host)
{
	...
	IP4_ADDR(&ip, 210,82,5,1);
	dns_setserver(1, &ip); 	
	//dns_gethostbyname(host, malloc(sizeof(struct ip_addr)), dns_found2, NULL);
	if (dns_gethostbyname(host, &ip, dns_found2, NULL) == ERR_OK)
	{
		rt_kprintf("IP hostname %s found ...\n", host);
	    rt_kprintf("IP: %d.%d.%d.%d \n",ip4_addr1(&ip),ip4_addr2(&ip),ip4_addr3(&ip),ip4_addr4(&ip));		
	}
...
#include <finsh.h>
FINSH_FUNCTION_EXPORT_ALIAS(open_url, open_url, open url )
FINSH_FUNCTION_EXPORT_ALIAS(get_hostip, get_hostip, get hostip )
#endif
